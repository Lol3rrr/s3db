...
impl Message {
    ...
    fn parse_internal(ty: u8, data: &[u8]) -> IResult<&[u8], Self> {
        match ty {
            ...
            b'E' => {
                let (rem, (portal, rows)) = nom::sequence::tuple((
                    parse_string,
                    nom::number::streaming::be_i32,
                ))(data)?;
                Ok((rem, Self::Execute { portal, max_rows: rows }))
            }
            b'P' => {
            ...
            b'B' => {
                let (rem, (destination, statement, mut pformats, pvalues, result_formats)) = nom::sequence::tuple((
                    parse_string,
                    parse_string,
            ...
            b'c' => {
                Ok((data, Self::CopyDone))
            }
            b'S' => {
                Ok((data, Self::Sync_))
            }
            b'X' => {
                Ok((data, Self::Terminate))
            }
            other => {
                tracing::error!("Unknown Raw-Type: {:?}", other);
                Err(nom::Err::Failure(nom::error::Error::new(data, nom::error::ErrorKind::Tag)))
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn parse_parse() {
        let (remaining, result) = Message::parse_internal(b'P', &[
    ...
    fn parse_bind_no_parameters() {
        let (remaining, result) = Message::parse_internal(b'B', &[
